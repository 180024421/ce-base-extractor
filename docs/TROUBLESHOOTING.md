...
- 放宽 GUI「最大层级」「最大偏移」（高级模式可见）
- 取消高级选项「交叉需全命中」；优先用 3 份 Rescan
- 确认 CE 导出的是指针扫描结果，而非普通地址列表

## Auto Script Studio 交接

- 使用「ASS 交接包」导出 Lua/SCC + 说明文件
- `bot.load_bases` / `mem.*` **仅 APK + root**；PC 联调会报不支持
- 先在 Studio 打包 debug APK，再推送脚本热替换
