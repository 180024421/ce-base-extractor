...
def list_ptrids(path: str | Path) -> list[int]:
    db_path = Path(path)
    conn = sqlite3.connect(f"file:{db_path.as_posix()}?mode=ro", uri=True)
    try:
        return _list_ptrids(conn)
    finally:
        conn.close()
...
def _row_to_chain(row: tuple, modules: dict[int, str]) -> PointerChain:
    module_id = int(row[0])
...
def _module_ids_clause(module_ids: set[int] | None) -> tuple[str, tuple]:
    if not module_ids:
        return "", ()
    placeholders = ",".join("?" * len(module_ids))
    return f" AND moduleid IN ({placeholders})", tuple(sorted(module_ids))


def iter_sqlite_chains(
    path: str | Path,
    ptrid: int | None = None,
    batch_size: int = 5000,
    ...
    try:
        ...
        extra_sql, extra_args = _module_ids_clause(module_ids)
        sql = (
            f"SELECT {', '.join(select_cols)} FROM results WHERE ptrid=?{extra_sql} "
            "ORDER BY resultid"
        )
        cur = conn.execute(sql, (resolved_ptrid, *extra_args))
        while True:
            rows = cur.fetchmany(batch_size)
            if not rows:
                break
            for row in rows:
                yield _row_to_chain(row, modules)
    finally:
        conn.close()
...
def load_sqlite(
    ...
    try:
        if not _table_exists(conn, "results"):
            raise ValueError(
                "数据库缺少 results 表，请使用 CE 菜单 File → Export to sqlite database 导出"
            )
        resolved_ptrid = _resolve_ptrid(conn, ptrid)
        modules = _load_modules(conn, resolved_ptrid)
        offset_cols = _offset_columns(conn)
        ...
        while True:
            ...
            if not rows:
                break
            for row in rows:
                chains.append(_row_to_chain(row, modules))
