...
def _read_bitfield(data: memoryview | bytes, bit_offset: int, bit_count: int) -> int:
    if bit_count <= 0:
        return 0
...
def _parse_header(data: memoryview | bytes) -> tuple[dict, int]:
    offset = 0
    version = data[offset]
    offset += 1
...
def _parse_plain_entry(
    data: memoryview | bytes, offset: int, header: dict
) -> tuple[PointerChain | None, int]:
...
def _iter_ptr_from_view(data: memoryview | bytes) -> Iterator[PointerChain]:
...
def iter_ptr_chains(path: str | Path) -> Iterator[PointerChain]:
    """mmap 流式迭代 .PTR，避免整文件 read_bytes。"""
    ptr_path = Path(path)
