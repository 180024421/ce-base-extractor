...
def _parse_header(data: bytes) -> tuple[dict, int]:
    ...
    external = struct.unpack_from("<I", data, offset)[0]
    offset += 4
    worker = struct.unpack_from("<I", data, offset)[0]
    offset += 4
...
def load_ptr(path: str | Path) -> tuple[list[PointerChain], dict]:
    ...
    entry_size_hint = (sum(header["max_bits"]) + 7) // 8 if header["compressed"] else (
        4 + 8 + 4 + header["maxlevel"] * 4
    )
