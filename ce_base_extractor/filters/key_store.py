...
class ChainKeyCounter:
    ...
    def _sqlite_upsert_batch(self, keys: dict[tuple, PointerChain]) -> None:
        conn = self._open_sqlite()
        rows = [(serialize_key(key), _chain_to_json(chain)) for key, chain in keys.items()]
        conn.executemany(
    ...
    def add_file_keys(self, keys: dict[tuple, PointerChain]) -> None:
        ...
        if self._use_sqlite:
            self._sqlite_upsert_batch(keys)
            return
    ...
    def iter_items_at_least(self, min_count: int) -> Iterator[tuple[PointerChain, int]]:
        if self._use_sqlite:
            conn = self._open_sqlite()
            for key_str, cnt, exemplar_json in conn.execute(
                "SELECT key, cnt, exemplar FROM chain_keys WHERE cnt >= ?", (min_count,)
            ):
                yield _chain_from_json(exemplar_json), int(cnt)
            return

        for key_str, cnt in self._memory_counter.items():
            if cnt >= min_count:
                yield self._memory_exemplar[key_str], int(cnt)

    def items_at_least(self, min_count: int) -> list[tuple[PointerChain, int]]:
        return list(self.iter_items_at_least(min_count))

    def count_at_least(self, min_count: int) -> int:
        if self._use_sqlite:
            conn = self._open_sqlite()
            row = conn.execute(
                "SELECT COUNT(*) FROM chain_keys WHERE cnt >= ?", (min_count,)
            ).fetchone()
            return int(row[0]) if row else 0
        return sum(1 for c in self._memory_counter.values() if c >= min_count)
    ...
    def close(self) -> None:
        path = self.db_path
        unlink = self.unlink_on_close
        if self._conn is not None:
            self._conn.close()
            self._conn = None
        if unlink and path and path != ":memory:":
            try:
                os.unlink(path)
            except OSError:
                pass
